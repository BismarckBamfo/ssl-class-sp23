...
def evaluate_model(model, dataloader, device):
    ...
    dev_accuracy = dev_accuracy.compute()
    return dev_accuracy
...
def train(mymodel, num_epochs, train_dataloader, validation_dataloader, device, lr):
    ...
    for epoch in range(num_epochs):
        ...
        acc.append(train_accuracy['accuracy'])
        ep.append(epoch+1)
        val.append(val_accuracy['accuracy'])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--model", nargs="+"  , type=str, default=["distilbert-base-uncased"])
    ...
    for x in args.model:
        results = []
        pretrained_model, train_dataloader, validation_dataloader, test_dataloader = pre_process(x,
                                                                                             args.batch_size,
                                                                                             args.device,
                                                                                             args.small_subset)
        ...
        for i in args.num_epochs:
            for j in args.lr: 
                ac, ep, val = train(pretrained_model, i, train_dataloader, validation_dataloader, args.device, j)

                # print the GPU memory usage just to make sure things are alright
                print_gpu_memory()

                val_accuracy = evaluate_model(pretrained_model, validation_dataloader, args.device)
                print(f" - Average DEV metrics for model {x} epoch {i} and lr {j}: accuracy={val_accuracy}")

                test_accuracy = evaluate_model(pretrained_model, test_dataloader, args.device)
                print(f" - Average TEST metrics for model {x} epoch {i} and lr {j}: accuracy={test_accuracy}")

                br1 = ["Dev", "Test"]
                

                #plotting the graph
                plt.bar(br1, [val_accuracy['accuracy'], test_accuracy['accuracy']], color=['r', 'b'], width=0.5)

                plt.title(f'Dev/Test Acc, model {x}, epochs = {i}, lr = {j}')
                ...
                # axis[0, 0].plot(ep, ac, color='r', label = "Training Accuracy")
                # axis[0, 1].plot(ep, val, color='b', label = "Validation Accuracy")
                # axis[0, 2].bar(val_accuracy['accuracy'], test_accuracy['accuracy'], color='maroon', width=0.5)

                # plt.title(f'Training/Dev/Test Accuracy for model {x} for epochs = {i} and lr = {j}')
                ...
                # axis[0, 1].legend()
                # axis[0, 2].xlabel('Dev Accuracy')


                # plt.title(f'Training/Dev Accuracy for model {x} for epochs = {i} and lr = {j}')
                # plt.xlabel('Epochs')
                # plt.ylabel('Accuracy')
                # plt.legend()
                ...
                #save the graph
                plt.savefig(f'/home/bodoom1/repos/csci-601-771-self-supervised-models/sp2023/hw6/train_dev_acc_q8_model_{x}_epoch_{i}_lr_{j}.png')
                plt.close()
