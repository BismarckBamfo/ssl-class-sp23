...
if __name__ == "__main__":
    ...
    print(f" >>>>>>>> Experiment {args.experiment}, subset={args.small_subset}, epochs={args.num_epochs}, lr={args.lr}, batch_size={args.batch_size}, device={args.device}, model={args.model}")
    ...
    for i in args.num_epochs:
        for j in args.lr: 
            ...
            print(f" - Average DEV metrics for epoch {i} and lr {j}: accuracy={val_accuracy}")

            test_accuracy = evaluate_model(pretrained_model, test_dataloader, args.device)
            print(f" - Average TEST metrics for epoch {i} and lr {j}: accuracy={test_accuracy}")

            #plotting the graph
            plt.plot(ep, ac, color='r', label = "Training Accuracy")
            plt.plot(ep, val, color='b', label = "Validation Accuracy")

            plt.title(f'Training/Dev Accuracy for epochs = {i} and lr = {j}')
            plt.xlabel('Epochs')
            plt.ylabel('Accuracy')
            plt.legend()
            ...
            #save the graph
            plt.savefig(f'/home/bodoom1/repos/csci-601-771-self-supervised-models/sp2023/hw6/train_dev_acc_q7_epoch_{i}_lr_{j}.png')
            plt.clf()
