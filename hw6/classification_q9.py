...
class T5(nn.Module):
    ...
    def forward(self, input_ids, attention_mask, decoder_input_ids):
        ...
        logits = self.classifier(torch.mean(output.last_hidden_state, dim=1))
        return logits
...
def pre_process(model_name, batch_size, device, small_subset=False):
    ...
    pretrained_model = T5(model_name, num_labels=2)
...
if __name__ == "__main__":
    ...
    for i in args.num_epochs:
        for j in args.lr: 
            ...
            print(f" - Average DEV metrics for model {args.model} epoch {i} and lr {j}: accuracy={val_accuracy}")

            test_accuracy = evaluate_model(pretrained_model, test_dataloader, args.device)
            print(f" - Average TEST metrics for model {args.model} epoch {i} and lr {j}: accuracy={test_accuracy}")
            ...
            plt.title(f'Dev/Test Acc, model {args.model}, epochs = {i}, lr = {j}')
            ...
            #save the graph
            plt.savefig(f'/home/bodoom1/repos/csci-601-771-self-supervised-models/sp2023/hw6/train_dev_acc_q9_model_{args.model}_epoch_{i}_lr_{j}.png')
            plt.close()
